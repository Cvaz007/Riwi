...
CREATE TABLE conjuntos (
  ...
  estrato INT NOT NULL CHECK(
    estrato BETWEEN 0 AND 6
  )
);
...
CREATE TABLE edificios (
  ...
  nro_de_pisos INT NOT NULL CHECK(
    nro_de_pisos BETWEEN 5 AND 8
  ),
...
ALTER TABLE edificios
ADD CONSTRAINT fk_id_conjunto foreign key(id_conjunto) REFERENCES conjuntos(nit_conjunto);
CREATE TABLE apartamentos (
  num_apto INT PRIMARY KEY AUTO_INCREMENT,
  piso INT NOT NULL,
  valor INT NOT NULL CHECK(
    valor BETWEEN 10000 AND 100000
  ),
  id_edificio INT NOT NULL,
  CONSTRAINT fk_id_edificio2 foreign key(id_edificio) REFERENCES edificios(id_edifico)
);
CREATE TABLE novedades(
  id INT PRIMARY KEY AUTO_INCREMENT,
  id_vigilante varchar(10) NOT NULL,
  id_edificio INT NOT NULL,
  CONSTRAINT fk_id_vigilante foreign key(id_vigilante) REFERENCES vigilantes(ced_vigilaten),
  CONSTRAINT fk_id_edificio foreign key(id_edificio) REFERENCES edificios(id_edifico)
);
ALTER TABLE edificios
